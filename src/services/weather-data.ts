...
class WeatherDataController {
    ...
    private apiKey: string = "3cac74d34bfca729805945914b0223f8"
    ...
    async refreshWeather() {
        ...
        try {
            if (location.useCoords) {
                ...
                response = await (fetch(`https://api.openweathermap.org/data/2.5/weather?q=${location.name}&APPID=${this.apiKey}`))
                if (!response.ok) {
                    throw new Error(response.statusText)
                }
            }
